...
> **注意**：本指南中呈现的所有视觉及音频素材，均由 Seedance/Seedream 系列视觉生成模型自主生成。
...
>   - 对于未定义主体的简单场景，每次提到主体时，都要使用 `<主体N>@<图片N>`，强调主体和素材的绑定关系。例如：张三@图片1。
>   - 对于已提前定义主体的场景，每次提到主体时，都应使用同一标签。例如：将**视频1**中的高个子男人定义为**警察**，将另一个矮个子男人定义为**小偷**。后续描述涉及到高个子男人持续使用"**警察**"指代，涉及到矮个子男人持续使用"**小偷**"指代。
> - 使用素材库（Asset ID）时，仍需使用 `<图片/视频N>` 指代主体。因模型无法直接关联 Asset ID 与参考内容，故不得直接以 Asset ID 替代 `<图片/视频N>`。
...
Seedance 2.0 系列模型支持生成常用文字。模型能根据情境**自动匹配**合适的风格与颜色，也支持在提示词中**指定**文字的颜色、风格、出现方式、出现时机、出现位置。编写时请优先使用**常用字**，避免**生僻字**与**特殊符号**，以确保最佳呈现效果。当前支持广告语、字幕、气泡等场景。
...
> **说明**：不建议用满素材上限，过多素材会导致模型难以判断特征优先级，易出现风格冲突、主体识别模糊、生成效果偏离预期等问题。
...
> **注意**：人物参考使用大头照 + 全身照即可，**不建议使用人物多视图**。多视图素材包含同一人物的不同角度，模型易将其识别为多个不同主体，反而加剧 ID 漂移问题。
...
> **说明**：视频帧对齐后，仍可能存在细微跳变，建议在续写生成视频的时候以转场切镜的时刻结尾，下一段视频以结尾切镜后的新场景为起始。
...
> **说明**：目前无法直接做到 100% 避免双胞胎问题，只能通过以下方法降低其出现概率、提升抽卡成功率。
...
> **说明**：Seedance 2.0 能根据情境匹配合适的文字风格，如果对文字表现效果的要求较为严格，可参考`多图参考 > Logo 参考`。
